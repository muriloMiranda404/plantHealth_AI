...
class AnalyticsTab extends StatelessWidget {
  ...
  final Widget Function(List<List<FlSpot>>, List<Color>, List<String>) chartBuilder;
        ...
        if (key == 'chart') {
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          color: comparisonSensors.isEmpty ? config.color : Colors.blueAccent,
                    ...
                    child: enabledSensorKeys.isEmpty
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                Icon(Icons.sensors_off, color: subTextColor, size: 36),
                            ...
                            comparisonSensors.isEmpty
                                ? [
                                    histories[activeGraphKey]!
                                        .map((spot) => FlSpot(spot.x, convertValue(activeGraphKey, spot.y, sensorUnits[activeGraphKey] ?? "")))
                                        .toList()
                                  ]
                                : comparisonSensors
                                    .where((k) => enabledSensorKeys.contains(k))
                                    .map((k) => histories[k]!
                                        .map((spot) => FlSpot(spot.x, convertValue(k, spot.y, sensorUnits[k] ?? "")))
                                        .toList())
                                    .toList(),
                            comparisonSensors.isEmpty
                                ? [config.color]
                                : comparisonSensors
                                    .where((k) => enabledSensorKeys.contains(k))
                                    .map((k) => getSensorConfig(k).color as Color)
                                    .toList(),
                            comparisonSensors.isEmpty
                                ? [activeGraphKey]
                                : comparisonSensors
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      TextButton.icon(
                        ...
                        label: const Text("Resetar", style: TextStyle(fontSize: 12)),
          ...
          return Column(
            ...
            children: [
              ...
              Text(
                "SELECIONE UM SENSOR",
                style: TextStyle(fontSize: 10, color: subTextColor.withValues(alpha: 0.5), letterSpacing: 1.2),
              ),
              const SizedBox(height: 10),
              Wrap(
                spacing: 10,
                runSpacing: 10,
                children: enabledSensorKeys.map((k) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: isSelected ? sCfg.color.withValues(alpha: 0.2) : Colors.transparent,
                        borderRadius: BorderRadius.circular(15),
                        border: Border.all(color: isSelected ? sCfg.color : subTextColor.withValues(alpha: 0.2)),
                      ),
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(sCfg.icon, size: 16, color: isSelected ? sCfg.color : subTextColor),
                          const SizedBox(width: 8),
                          Text(sCfg.name, style: TextStyle(color: isSelected ? textColor : subTextColor, fontSize: 12, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
                        ],
                      ),
                    ),
                  );
